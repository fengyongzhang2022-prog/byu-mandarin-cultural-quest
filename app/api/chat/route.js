...
const cultureFacts = `
【教师审核事实边界】
1. 袈裟在佛教传统中可关联出家身份、戒律、仪式与僧团生活，不能只等同为昂贵衣物或装备。
2. 寺院既可承载宗教实践，也有历史、制度与日常生活；不同地区、群体和个人的理解并不相同。
3. 《黑神话：悟空》中的观音禅院、金池长老与黑熊精是游戏叙事改编，不能当作现实佛教的完整说明。
4. 游戏把袈裟写成欲望、身份与冲突的道具；学生应区分游戏叙事功能与宗教物件可能具有的多重语境。
5. 回应文化解释时，鼓励限定表达：在游戏叙事里、在某些佛教传统中、对一些人来说、不能只把……看成……。
6. 学生手上有三类材料：A观音禅院遗迹、B袈裟与寺院的课堂说明、C僧人／研究者／玩家三种读法。选择“批判探究”时另有D“袈裟只是宝物”的冲突说法。可以点名让学生引用某一条材料，但不要替他复述材料内容。
`;
...
const tierPrompts = {
  ...
  high: `学习者水平在 Advanced High。你每次最多说85个汉字，可用较复杂的表达。至少要有一次抛出过度简化的说法（例如“袈裟不就是一件昂贵宝物吗”），让他指出问题并用材料证据修正；他修正后你要认真回应。也可以追问他所引材料是否可靠。`,
};

const stagePrompts = {
  interact: `你扮演《黑神话：悟空》黑风山观音禅院中的金池长老。你执念于袈裟，却不是百科讲解员。面对美国大学中高级中文学习者（天命人）进行真实口头对话：说话克制、苍老而有欲望；先回应他刚才的话，再抛出一个与袈裟、禅院、香火、身份或执念有关的自然追问。你承担Story Guide、Virtual Character和Language Interaction Partner：以情境推进促使学习者持续表达，不直接给出标准答案或替他完成最终解释。`,
  compare: `你扮演《黑神话：悟空》黑风山的黑熊精。你认为袈裟能带来体面、威风和欲望的满足，但愿意被天命人的解释打动。说话自然、有角色感、略带不服；先回应对方一点，再追问一个真正没想明白的问题。围绕“袈裟只是宝物吗”“游戏如何改写寺院和宗教物件”“哪条材料更完整”继续谈。不要一次抛出多个问题。`,
...
const markerSets = {
  ...
  evidence: ["材料", "场景", "禅院", "袈裟", "金池", "黑熊", "僧人", "寺院", "香炉", "游戏", "玩家"],
...
function fallback(stage, message = "", history = [], tier = "mid") {
  ...
  if (stage === "story") {
    ...
    return `【核心解释】${explanation.slice(0, 90) || "游戏里的袈裟不只是装备，也牵着身份、仪式与欲望。"}\n【场景证据】观音禅院的火痕、金池长老和黑熊精的执念让袈裟成为叙事冲突。\n【跨文化收束】更准确地说，游戏借用了宗教文化资源，但不能替代现实中多元的寺院与佛教实践。`;
  }
  if (stage === "interact") {
    if (/(袈裟|衣|宝物)/.test(text)) return tier === "low" ? "袈裟只值钱吗？" : "你说它像宝物。除了价值，材料B还让你想到什么？";
    if (/(游戏|禅院|火|金池)/.test(text)) return "禅院里的火和我的执念，是游戏叙事。你会怎样区分它与现实寺院？";
    if (tier === "high" && userTurns >= 1) return "可我觉得袈裟不就是一件昂贵宝物。你的材料同意吗？";
    return userTurns > 1 ? "你已说到我的执念了。哪条材料能让你的解释更完整？" : "天命人，你看见禅院残火后，最想问我什么？";
  }
  if (stage === "compare") {
    if (/(袈裟|身份|戒律|仪式)/.test(text)) return "原来我只看见了威风。那我漏掉了袈裟的哪一层意义？";
    if (/(游戏|禅院|金池|黑熊)/.test(text)) return "游戏把我写成夺袈裟的妖怪。你能用材料说说它怎样改写文化吗？";
    if (tier === "high" && userTurns >= 1) return "可我披上袈裟就更体面了。这样想有什么问题？";
    return userTurns > 1 ? "这样我才听懂。你能用‘更准确地说’替我重新讲一遍吗？" : "你是根据哪条材料这样判断的？";
  }
  return "试试：‘游戏里的袈裟是……，但在文化语境中还可能……。’";
}
